...
class LogFile:
    """An object representing a log file. Most functions use `self.work_set`,
    original logs sorted in `self.logs`.
    ...
    @staticmethod
    def from_logs_link(link: str, logs_we_care_about: list[str] = None,
                       verbose: bool = False, quiet: bool = False) -> LogFile:
        ...
        Parameters:
        `logs_we_care_about` (list[str]): list of strings, containing the file
            names. For example: `["game.txt", "attack.txt"]`.This defaults to
            all supported files.
        `verbose` (bool): toggle verbose mode (False by default)
        `quiet` (bool): toggle quiet mode (False by default)
        ...
        # Should be all supported log types as a default.
        # Don't forget to update this list! (you will)
        if not logs_we_care_about:
            logs_we_care_about = ["game.txt", "attack.txt", "pda.txt",
                                  "silicon.txt", "mecha.txt", "virus.txt",
                                  "telecomms.txt", "uplink.txt", "shuttle.txt"]
        if link[-1] != "/":
            link += "/"
        log_collection = LogFile()
